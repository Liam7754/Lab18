...
def run_tests() -> tuple[int, int]:
    ...
    try:
        ...
        # Pytest summary lines vary across versions — search the whole output
        # for "<n> passed" / "<n> failed" / "<n> errors" with regex.
        out = result.stdout + "\n" + result.stderr
        passed = sum(int(m) for m in re.findall(r"(\d+)\s+passed", out))
        failed = sum(int(m) for m in re.findall(r"(\d+)\s+failed", out))
        errors = sum(int(m) for m in re.findall(r"(\d+)\s+error", out))
